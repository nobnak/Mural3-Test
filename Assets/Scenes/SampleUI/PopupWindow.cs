...
public class PopupWindow : TitleContent<PopupWindow>, IDoGUIWindow {
    public Rect rect;
    ...
    public PopupWindow(string name, float width = 300f) : base(name) {
        rect.width = width;
    }

    public PopupWindow SetWidth(float width) {
        rect.width = width;
        return this;
    }

    public PopupWindow SetHeight(float height) {
        rect.height = height;
        return this;
    }
    public PopupWindow SetOpen(bool open) {
        isOpen = open;
        if (isOpen) {
            isMoved = false;
    ...
    public void DoGUI() {
        if (isEnable && isOpen) WindowInvoker.Add(this);
    }
